...
public class BaseActor extends Group
{
    ...
    public void applyLinearImpulse(float ix, float iy)
    {
    ...
    public void setSpeed(float speed) {
        Vector2 velocity = body.getLinearVelocity().nor();
        velocity.scl(speed);
    ...
    public void createDynamicBody() {
        ...
        // Define the box dimensions based on the desired width and height
        polygon.setAsBox(getWidth() / 2, getHeight() / 2); // Half width and half height
    ...
    public void createStaticBody() {
        ...
        // Define a polygon shape using setAsBox
        PolygonShape polygon = new PolygonShape();
        // Set as box takes half-width and half-height
        polygon.setAsBox(getWidth() / 2, getHeight() / 2);

        // Define the fixture
        FixtureDef fixtureDef = new FixtureDef();
        fixtureDef.shape = polygon;
        fixtureDef.density = 0;  // Density is often irrelevant for static bodies
        fixtureDef.friction = 0.5f;
        fixtureDef.restitution = 0.6f;
     ...
     *  BaseActor whose size determines the world bounds (typically a background image)
     */
    ...
    public void alignCamera() {
        ...
        // ensure camera does not exceed world boundaries
        float halfViewportWidth = camera.viewportWidth * 0.5f;
        float halfViewportHeight = camera.viewportHeight * 0.5f;
        camera.position.x = MathUtils.clamp(camera.position.x, halfViewportWidth, worldBounds.width - halfViewportWidth);
        camera.position.y = MathUtils.clamp(camera.position.y, halfViewportHeight, worldBounds.height - halfViewportHeight);
